...
from models.credit import CreditScore, CreditHistory, CreditEventType
...
class TestCreditScoringService:
    ...
    def test_calculate_credit_score_existing_user(self, credit_service, db, sample_user, sample_credit_score):
        ...
        result = credit_service.calculate_credit_score(sample_user.id, force_recalculation=True)
        
        assert 'score' in result
        assert isinstance(result['score'], int)
        
        # Score might change based on new history
        new_score = CreditScore.query.filter_by(user_id=sample_user.id).order_by(CreditScore.calculated_at.desc()).first()
        assert new_score is not None
    
    def test_calculate_credit_score_with_blockchain(self, credit_service, db, sample_user):
    ...
    def test_get_credit_score_current(self, credit_service, db, sample_user, sample_credit_score):
    ...
    def test_get_credit_recommendations(self, credit_service, db, sample_user):
        ...
        payment_rec = next((r for r in recommendations if 'payment' in r['title'].lower()), None)
        assert payment_rec is not None
        assert 'priority' in payment_rec
        assert 'description' in payment_rec
    
    def test_simulate_score_impact(self, credit_service, db, sample_user, sample_credit_score):
    ...
    def test_credit_score_caching(self, credit_service, db, sample_user, sample_credit_score):
    ...
    def test_credit_monitoring_alerts(self, credit_service, db, sample_user, sample_credit_score):
        ...
        with patch.object(credit_service, '_send_alert') as mock_alert:
            credit_service._check_score_alerts(sample_user.id, new_score, original_score)
    ...
    def test_credit_report_generation(self, credit_service, db, sample_user, sample_credit_score):
    ...
    def test_performance_monitoring(self, credit_service, db, sample_user):
        ...
        timing_call = next((call for call in metric_calls if 'duration' in call[0][0]), None)
