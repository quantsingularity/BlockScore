...
class TestMFAService:
    ...
    def test_setup_totp_success(self, mfa_service, mock_user):
        ...
        with patch('services.mfa_service.User') as MockUser, \
             patch.object(mfa_service, '_generate_qr_code') as mock_qr, \
             patch.object(mfa_service, '_encrypt_secret') as mock_encrypt:
    ...
    def test_verify_totp_setup_success(self, mfa_service, mock_user):
        ...
        with patch('services.mfa_service.User') as MockUser, \
             patch.object(mfa_service, '_decrypt_secret') as mock_decrypt, \
             patch.object(mfa_service, '_generate_backup_codes') as mock_backup:
    ...
    def test_setup_sms_mfa_success(self, mfa_service, mock_user):
        ...
        with patch('services.mfa_service.User') as MockUser, \
             patch.object(mfa_service, '_validate_phone_number') as mock_validate, \
             patch.object(mfa_service, '_generate_sms_code') as mock_code, \
             patch.object(mfa_service, '_send_sms') as mock_sms, \
             patch.object(mfa_service, '_encrypt_secret') as mock_encrypt:
    ...
    def test_verify_sms_setup_success(self, mfa_service, mock_user):
        ...
        mock_user.profile.sms_code_expires_at = datetime.now(timezone.utc) + timedelta(minutes=5)
    ...
    def test_verify_sms_setup_expired_code(self, mfa_service, mock_user):
        ...
        verification_code = '123456'
        mock_user.profile.sms_verification_code = 'encrypted_code'
        mock_user.profile.sms_code_expires_at = datetime.now(timezone.utc) - timedelta(minutes=1)  # Expired
    ...
    def test_verify_mfa_totp_success(self, mfa_service, mock_user):
        ...
        with patch('services.mfa_service.User') as MockUser, \
             patch.object(mfa_service, '_is_mfa_locked') as mock_locked, \
             patch.object(mfa_service, '_verify_totp_code') as mock_verify, \
             patch.object(mfa_service, '_reset_mfa_failed_attempts') as mock_reset:
    ...
    def test_verify_mfa_backup_code_success(self, mfa_service, mock_user):
        ...
        with patch('services.mfa_service.User') as MockUser, \
             patch.object(mfa_service, '_is_mfa_locked') as mock_locked, \
             patch.object(mfa_service, '_verify_backup_code') as mock_verify, \
             patch.object(mfa_service, '_reset_mfa_failed_attempts') as mock_reset:
    ...
    def test_verify_mfa_invalid_code(self, mfa_service, mock_user):
        ...
        with patch('services.mfa_service.User') as MockUser, \
             patch.object(mfa_service, '_is_mfa_locked') as mock_locked, \
             patch.object(mfa_service, '_verify_totp_code') as mock_verify, \
             patch.object(mfa_service, '_increment_mfa_failed_attempts') as mock_increment:
    ...
    def test_send_sms_code_success(self, mfa_service, mock_user):
        ...
        with patch('services.mfa_service.User') as MockUser, \
             patch.object(mfa_service, '_is_sms_rate_limited') as mock_rate_limit, \
             patch.object(mfa_service, '_generate_sms_code') as mock_code, \
             patch.object(mfa_service, '_decrypt_secret') as mock_decrypt, \
             patch.object(mfa_service, '_send_sms') as mock_sms, \
             patch.object(mfa_service, '_encrypt_secret') as mock_encrypt, \
             patch.object(mfa_service, '_update_sms_rate_limit') as mock_update:
    ...
    def test_get_mfa_status(self, mfa_service, mock_user):
        ...
        with patch('services.mfa_service.User') as MockUser, \
             patch.object(mfa_service, '_decrypt_secret') as mock_decrypt, \
             patch.object(mfa_service, '_mask_phone_number') as mock_mask:
    ...
    def test_regenerate_backup_codes(self, mfa_service, mock_user):
        ...
        with patch('services.mfa_service.User') as MockUser, \
             patch.object(mfa_service, '_generate_backup_codes') as mock_generate:
