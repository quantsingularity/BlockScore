...
from models.audit import ComplianceRecord, ComplianceType, ComplianceStatus
from models.user import User, UserProfile, KYCStatus
...
class ComplianceService:
    ...
    def __init__(self, db):
        ...
        self.kyc_requirements = {
            ...
            'enhanced': ['full_name', 'date_of_birth', 'address', 'government_id', 'income_verification'],
            'premium': ['full_name', 'date_of_birth', 'address', 'government_id', 'income_verification', 'employment_verification', 'bank_statements']
        }
    ...
    def perform_kyc_assessment(self, user_id: str, kyc_level: str = 'basic') -> Dict[str, Any]:
        ...
        try:
            ...
            requirements = self.kyc_requirements.get(kyc_level, self.kyc_requirements['basic'])
            ...
            kyc_status = self._determine_kyc_status(compliance_score, assessment_results)
            ...
            if kyc_status != profile.kyc_status:
                ...
                profile.kyc_completed_at = datetime.now(timezone.utc) if kyc_status == KYCStatus.VERIFIED else None
            ...
            return {
                ...
                'next_review_date': compliance_record.next_review_date.isoformat() if compliance_record.next_review_date else None
            }
    ...
    def perform_aml_screening(self, user_id: str, transaction_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            screening_results = {
                ...
                'transaction_monitoring': self._monitor_transaction_patterns(user_id, transaction_data),
            ...
            compliance_record = self._create_compliance_record(
                ...
                entity_id=user_id,
                status=status,
                compliance_score=100 - risk_score,  # Invert risk score for compliance score
            ...
            return {
                ...
                'recommended_actions': self._get_aml_recommended_actions(screening_results, risk_score)
            }
    ...
    def assess_loan_compliance(self, loan_application_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Determine compliance status
            status = ComplianceStatus.COMPLIANT if compliance_score >= 85 else ComplianceStatus.NON_COMPLIANT
            ...
            return {
                ...
                'required_actions': self._get_loan_compliance_actions(compliance_checks)
            }
            
        except Exception as e:
            self.logger.error(f"Loan compliance assessment failed for application {loan_application_id}: {e}")
            raise e
    
    def monitor_ongoing_compliance(self, entity_type: str, entity_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            records = ComplianceRecord.query.filter_by(
                ...
                entity_id=entity_id
            ).order_by(ComplianceRecord.assessed_at.desc()).all()
            
            # Check for expired or soon-to-expire records
            now = datetime.now(timezone.utc)
            expired_records = [r for r in records if r.valid_until and r.valid_until < now]
            expiring_records = [r for r in records if r.next_review_date and r.next_review_date < now + timedelta(days=30)]
            ...
            self.logger.error(f"Compliance monitoring failed for {entity_type} {entity_id}: {e}")
            raise e
    
    def generate_compliance_report(self, start_date: datetime, end_date: datetime,
                                 compliance_types: List[ComplianceType] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if compliance_types:
                query = query.filter(ComplianceRecord.compliance_type.in_(compliance_types))
            ...
            report = {
                ...
                'summary': {
                    ...
                    'non_compliant': len([r for r in records if r.status == ComplianceStatus.NON_COMPLIANT]),
                    'pending_review': len([r for r in records if r.status == ComplianceStatus.PENDING_REVIEW]),
                    'average_score': sum(r.compliance_score or 0 for r in records) / len(records) if records else 0
                },
    ...
    def _assess_kyc_requirements(self, profile: UserProfile, requirements: List[str]) -> Dict[str, Any]:
        ...
        for requirement in requirements:
            if requirement == 'full_name':
                results['full_name'] = {
                    ...
                    'details': 'Full name provided' if (profile.first_name and profile.last_name) else 'Full name missing'
                }
                ...
                results['date_of_birth'] = {
                    ...
                    'details': 'Date of birth provided' if profile.date_of_birth else 'Date of birth missing'
                }
            elif requirement == 'address':
                has_address = bool(profile.address_line1 and profile.city and profile.country)
                results['address'] = {
                    'satisfied': has_address,
                    'score': 100 if has_address else 0,
                    'details': 'Address provided' if has_address else 'Complete address missing'
                }
                ...
                results['government_id'] = {
                    ...
                    'details': 'Government ID provided' if profile.government_id_number else 'Government ID missing'
                }
            elif requirement == 'income_verification':
                results['income_verification'] = {
                    'satisfied': bool(profile.annual_income and profile.employment_status),
                    'score': 80 if (profile.annual_income and profile.employment_status) else 0,
                    'details': 'Income information provided' if (profile.annual_income and profile.employment_status) else 'Income verification missing'
                }
            else:
                results[requirement] = {
                    'satisfied': False,
    ...
    def _determine_kyc_status(self, compliance_score: float, assessment_results: Dict[str, Any]) -> KYCStatus:
    ...
    def _check_sanctions_list(self, user: User) -> Dict[str, Any]:
        ...
        full_name = f"{profile.first_name or ''} {profile.last_name or ''}".strip().upper()
    ...
    def _monitor_transaction_patterns(self, user_id: str, transaction_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """Monitor transaction patterns for suspicious activity"""
        # Get recent credit history (proxy for transactions)
        recent_history = CreditHistory.query.filter_by(user_id=user_id)\
            .filter(CreditHistory.event_date > datetime.now(timezone.utc) - timedelta(days=30))\
            .all()
    ...
    def _analyze_user_behavior(self, user_id: str) -> Dict[str, Any]:
        ...
        history = CreditHistory.query.filter_by(user_id=user_id)\
            .order_by(CreditHistory.event_date.desc()).limit(100).all()
        ...
        recent_events = [h for h in history if h.event_date > datetime.now(timezone.utc) - timedelta(days=7)]
    ...
    def _determine_aml_status(self, risk_score: float, screening_results: Dict[str, Any]) -> ComplianceStatus:
    ...
    def _check_fair_lending_compliance(self, loan_app: LoanApplication) -> Dict[str, Any]:
    ...
    def _check_usury_compliance(self, loan_app: LoanApplication) -> Dict[str, Any]:
        ...
        return {
            ...
            'notes': 'Interest rate within legal limits' if compliant else f'Rate {current_rate}% exceeds limit {max_rate}%'
        }
    
    def _calculate_loan_compliance_score(self, compliance_checks: Dict[str, Any]) -> float:
        """Calculate overall loan compliance score"""
        scores = [check['score'] for check in compliance_checks.values()]
        return sum(scores) / len(scores) if scores else 0
    
    def _create_compliance_record(self, compliance_type: ComplianceType, entity_type: str,
                                entity_id: str, status: ComplianceStatus, compliance_score: float,
                                assessment_data: Dict[str, Any], regulation_name: str,
                                requirement_description: str) -> ComplianceRecord:
    ...
    def _get_kyc_required_actions(self, assessment_results: Dict[str, Any]) -> List[str]:
    ...
    def _get_aml_recommended_actions(self, screening_results: Dict[str, Any], risk_score: float) -> List[str]:
    ...
    def _check_sar_requirements(self, risk_score: float, screening_results: Dict[str, Any]) -> bool:
        """Check if Suspicious Activity Report (SAR) is required"""
        return risk_score > 80 or len(screening_results['sanctions_check']['matches']) > 0
    ...
    def _identify_compliance_violations(self, compliance_checks: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for check_name, check_result in compliance_checks.items():
            if not check_result.get('compliant', True):
                violations.append({
                    'type': check_name,
                    'severity': 'high' if check_result.get('score', 0) < 50 else 'medium',
                    'description': check_result.get('notes', f'{check_name} compliance issue'),
                    'score': check_result.get('score', 0)
                })
        return violations
    
    def _get_loan_compliance_actions(self, compliance_checks: Dict[str, Any]) -> List[str]:
        """Get required actions for loan compliance"""
        actions = []
        for check_name, check_result in compliance_checks.items():
            if not check_result.get('compliant', True):
                actions.append(f"Address {check_name.replace('_', ' ')} compliance issue")
        return actions
    
    def _calculate_compliance_health(self, records: List[ComplianceRecord]) -> Dict[str, Any]:
    ...
    def _generate_compliance_summary(self, records: List[ComplianceRecord]) -> Dict[str, Any]:
        ...
        for record in records:
            ...
            summary[comp_type]['status_counts'][status] = summary[comp_type]['status_counts'].get(status, 0) + 1
    ...
    def _group_by_compliance_type(self, records: List[ComplianceRecord]) -> Dict[str, Any]:
    ...
    def _analyze_compliance_trends(self, records: List[ComplianceRecord]) -> Dict[str, Any]:
        ...
        older_scores = [r.compliance_score or 0 for r in sorted_records[:-10]] if len(sorted_records) > 10 else []
    ...
    def _generate_compliance_recommendations(self, records: List[ComplianceRecord]) -> List[str]:
        ...
        kyc_issues = [r for r in non_compliant if r.compliance_type == ComplianceType.KYC]
        if kyc_issues:
            recommendations.append("Improve KYC data collection processes")
        
        aml_issues = [r for r in non_compliant if r.compliance_type == ComplianceType.AML]
        if aml_issues:
