...
from models.credit import (
    ...
    CreditScoreStatus, CreditFactorType, CreditEventType
)
...
class CreditScoringService:
    ...
    def calculate_credit_score(self, user_id: str, wallet_address: str = None, 
        ...
        try:
            ...
            self.logger.error(f"Credit score calculation failed for user {user_id}: {e}")
            raise e
    ...
    def simulate_score_impact(self, user_id: str, scenario: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def update_credit_event(self, user_id: str, event_type: CreditEventType, 
        ...
        try:
            ...
            event = CreditHistory(
                ...
                event_title=event_data.get('title', event_type.value.replace('_', ' ').title()),
    ...
    def _load_model(self):
        ...
        try:
            ...
            self.logger.warning(f"Could not load AI model: {e}. Using rule-based scoring.")
            self.model = None
    ...
    def _get_recent_valid_score(self, user_id: str) -> Optional[CreditScore]:
        ...
        return CreditScore.query.filter_by(user_id=user_id)\
    ...
    def _gather_scoring_data(self, user: User, wallet_address: str = None) -> Dict[str, Any]:
        """Gather all data needed for credit scoring"""
        data = {
            'user_id': user.id,
            'wallet_address': wallet_address or (user.profile.wallet_address if user.profile else None),
            'profile_data': {},
            'credit_history': [],
            'blockchain_data': {},
            'financial_data': {}
        ...
        if user.profile:
            data['profile_data'] = {
                'annual_income': float(user.profile.annual_income) if user.profile.annual_income else None,
        ...
        credit_events = CreditHistory.query.filter_by(user_id=user.id)\
    ...
    def _get_blockchain_data(self, wallet_address: str) -> Dict[str, Any]:
        ...
        ).order_by(BlockchainTransaction.submitted_at.desc()).limit(100).all()
        ...
        return {
            ...
            'avg_transaction_amount': total_volume / len(transactions) if transactions else 0,
    ...
    def _calculate_credit_utilization_factor(self, data: Dict[str, Any]) -> CreditFactor:
    ...
    def _calculate_blockchain_activity_factor(self, data: Dict[str, Any]) -> CreditFactor:
    ...
    def _calculate_overall_score(self, factors: List[CreditFactor]) -> int:
        ...
        if self.model:
            ...
            score = self.min_score + (total_contribution / 100) * (self.max_score - self.min_score)
    ...
    def _calculate_score_with_model(self, factors: List[CreditFactor]) -> int:
        ...
        try:
            ...
            feature_dict = {factor.factor_type.value: factor.normalized_value for factor in factors}
    ...
    def _create_credit_score_record(self, user_id: str, score: int, 
                                  factors: List[CreditFactor], 
                                  scoring_data: Dict[str, Any]) -> CreditScore:
    ...
    def _create_credit_history_event(self, user_id: str, credit_score_id: str,
                                   event_type: CreditEventType, score_after: int):
        """Create credit history event"""
        try:
            event = CreditHistory(
                id=str(uuid.uuid4()),
    ...
    def _format_score_response(self, credit_score: CreditScore) -> Dict[str, Any]:
        ...
        return {
            ...
            'expires_at': credit_score.expires_at.isoformat() if credit_score.expires_at else None,
    ...
    def _apply_scenario_changes(self, user_id: str, scenario: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _analyze_score_impact(self, factors: List[CreditFactor], scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for factor in factors:
            if factor.normalized_value < 0.6:  # Below 60%
                if factor.factor_type == CreditFactorType.PAYMENT_HISTORY:
                    ...
                    recommendations.append("Increase blockchain transaction activity and reliability")
