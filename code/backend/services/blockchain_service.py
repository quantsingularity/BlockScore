...
from models.blockchain import (
    BlockchainTransaction, SmartContract, TransactionType, 
    TransactionStatus, ContractType, ContractStatus
)
...
class BlockchainService:
    ...
    def __init__(self, config):
        ...
        self.contract_addresses = {
            ...
            ContractType.IDENTITY_REGISTRY: config.get('IDENTITY_REGISTRY_CONTRACT_ADDRESS'),
            ContractType.PAYMENT_PROCESSOR: config.get('PAYMENT_PROCESSOR_CONTRACT_ADDRESS')
        }
    ...
    def submit_credit_score_update(self, user_id: str, credit_score_id: str, 
                                 score: int, wallet_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            transaction = contract.functions.updateCreditScore(
                ...
                'nonce': self.web3.eth.get_transaction_count(self.config.get('BLOCKCHAIN_FROM_ADDRESS'))
            })
            
            # Sign and send transaction
            signed_txn = self.web3.eth.account.sign_transaction(
                transaction, 
                private_key=self.config.get('BLOCKCHAIN_PRIVATE_KEY')
            )
            ...
            return {
                ...
                'transaction_hash': tx_hash_hex,
                'status': 'submitted',
                'estimated_confirmation_time': self.confirmation_blocks * 15  # ~15 seconds per block
            }
    ...
    def submit_loan_agreement(self, loan_id: str, borrower_address: str, 
                            loan_amount: Decimal, interest_rate: float,
    ...
    def record_payment(self, loan_id: str, payment_amount: Decimal, 
                      borrower_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            blockchain_tx = self._create_blockchain_transaction(
                ...
                contract_address=self.contract_addresses.get(ContractType.PAYMENT_PROCESSOR),
    ...
    def get_transaction_status(self, transaction_hash: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Determine status
                if receipt.status == 1:
                    status = 'confirmed' if confirmations >= self.confirmation_blocks else 'pending'
                else:
                    status = 'failed'
    ...
    def update_transaction_status(self, transaction_id: str) -> bool:
        ...
        try:
            ...
            if status_info['status'] == 'confirmed':
                ...
                blockchain_tx.error_message = status_info.get('error', 'Transaction failed')
    ...
    def get_wallet_transaction_history(self, wallet_address: str, 
        ...
        try:
            ...
            ).order_by(BlockchainTransaction.submitted_at.desc()).limit(limit).all()
            
            # If no database records and blockchain is connected, try to fetch from blockchain
            if not transactions and self.is_connected():
                transactions = self._fetch_wallet_history_from_blockchain(wallet_address, limit)
    ...
    def deploy_smart_contract(self, contract_type: ContractType, 
                            contract_name: str, bytecode: str, 
                            abi: List[Dict], constructor_args: List = None) -> Dict[str, Any]:
        ...
        try:
            ...
            transaction = contract.constructor(*constructor_args).build_transaction({
                'from': self.config.get('BLOCKCHAIN_FROM_ADDRESS'),
                'gas': self.default_gas_limit * 3,  # Higher gas limit for deployment
                'gasPrice': self.default_gas_price,
                'nonce': self.web3.eth.get_transaction_count(self.config.get('BLOCKCHAIN_FROM_ADDRESS'))
            })
    ...
    def get_contract_info(self, contract_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            contract = SmartContract.query.filter_by(contract_address=contract_address).first()
    ...
    def estimate_gas(self, contract_address: str, function_name: str, 
                    function_args: List = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get contract instance
            contract_record = SmartContract.query.filter_by(contract_address=contract_address).first()
            if not contract_record:
                return {'error': 'Contract not found'}
            
            contract = self.web3.eth.contract(
                address=contract_address,
                abi=contract_record.get_abi()
            )
    ...
    def _get_contract_instance(self, contract_type: ContractType):
        ...
        try:
            ...
            self.logger.error(f"Failed to get contract instance for {contract_type}: {e}")
            return None
    
    def _create_blockchain_transaction(self, transaction_hash: str, transaction_type: TransactionType,
                                     from_address: str, to_address: str = None, 
                                     contract_address: str = None, function_name: str = None,
                                     ...
                                     value: int = 0) -> BlockchainTransaction:
    ...
    def _fetch_wallet_history_from_blockchain(self, wallet_address: str, 
                                            limit: int = 100) -> List[BlockchainTransaction]:
    ...
    def monitor_pending_transactions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pending_txs = BlockchainTransaction.query.filter_by(
                ...
                BlockchainTransaction.submitted_at > datetime.now(timezone.utc) - timedelta(hours=24)
            ).all()
    ...
    def get_blockchain_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            confirmed_txs = BlockchainTransaction.query.filter_by(status=TransactionStatus.CONFIRMED).count()
            failed_txs = BlockchainTransaction.query.filter_by(status=TransactionStatus.FAILED).count()
            pending_txs = BlockchainTransaction.query.filter_by(status=TransactionStatus.PENDING).count()
            ...
            active_contracts = SmartContract.query.filter_by(status=ContractStatus.ACTIVE).count()
            ...
            return {
                ...
                'transaction_stats': {
                    ...
                    'success_rate': (confirmed_txs / total_txs * 100) if total_txs > 0 else 0
                },
                'contract_stats': {
