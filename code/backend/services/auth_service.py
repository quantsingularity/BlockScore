...
from models.user import User, UserProfile, UserSession, UserStatus, KYCStatus
...
class AuthService:
    ...
    def authenticate_user(self, email: str, password: str, ip_address: str = None, 
                         user_agent: str = None, mfa_code: str = None) -> Optional[User]:
    ...
    def disable_mfa(self, user_id: str, password: str, verification_code: str = None) -> bool:
    ...
    def _is_suspicious_login(self, user: User, ip_address: str, user_agent: str) -> bool:
        ...
        recent_sessions = UserSession.query.filter_by(user_id=user.id)\
            .filter(UserSession.created_at > datetime.now(timezone.utc) - timedelta(days=30))\
        ...
        recent_ips = {session.ip_address for session in recent_sessions if session.ip_address}
        if ip_address not in recent_ips and len(recent_ips) > 0:
            return True
        
        # Check for unusual user agent
        recent_agents = {session.user_agent for session in recent_sessions if session.user_agent}
    ...
    def _generate_device_fingerprint(self, user_agent: str = None, ip_address: str = None) -> str:
        ...
        fingerprint_data = f"{user_agent or ''}{ip_address or ''}{datetime.now().date()}"
    ...
    def _log_security_alert(self, user_id: str, alert_type: str, details: Dict[str, Any]):
        """Log security alerts"""
        try:
            audit_log = AuditLog(
                id=str(uuid.uuid4()),
    ...
    def get_security_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        recent_logins = UserSession.query.filter_by(user_id=user_id)\
            .filter(UserSession.created_at > datetime.now(timezone.utc) - timedelta(days=30))\
            .count()
