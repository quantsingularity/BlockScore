...
class LoanApplication(db.Model):
    ...
    # Application Data
    application_data = db.Column(db.Text, nullable=True)  # JSON with application details
...
class Loan(db.Model):
    ...
    def calculate_remaining_balance(self):
        ...
        total_payments = sum(payment.amount for payment in self.payments if payment.status == PaymentStatus.COMPLETED)
...
class LoanPayment(db.Model):
    ...
    processor_response = db.Column(db.Text, nullable=True)  # JSON with processor response
...
class LoanApplicationSchema(Schema):
    ...
    loan_type = fields.Str(required=True, validate=validate.OneOf([e.value for e in LoanType]))
    requested_amount = fields.Decimal(required=True, places=2, validate=validate.Range(min=100))
    requested_term_months = fields.Int(required=True, validate=validate.Range(min=1, max=360))
    requested_rate = fields.Float(validate=validate.Range(min=0, max=100), allow_none=True)
...
class LoanPaymentSchema(Schema):
    ...
    payment_method = fields.Str(required=True, validate=validate.OneOf([e.value for e in PaymentMethod]))
