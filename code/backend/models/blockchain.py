...
class BlockchainTransaction(db.Model):
    ...
    transaction_fee = db.Column(db.Decimal(20, 8), nullable=True)  # In ETH or native token
    ...
    # Network Information
    network_id = db.Column(db.Integer, nullable=False, default=1)  # 1 for Ethereum mainnet
    ...
    def is_confirmed(self):
        ...
        return self.status == TransactionStatus.CONFIRMED and self.confirmation_count >= self.required_confirmations
...
class SmartContract(db.Model):
    ...
    audit_reports = db.Column(db.Text, nullable=True)  # JSON array of audit report references
    known_vulnerabilities = db.Column(db.Text, nullable=True)  # JSON array of vulnerabilities
    ...
    def set_known_vulnerabilities(self, vulnerabilities):
        ...
        self.known_vulnerabilities = json.dumps(vulnerabilities) if vulnerabilities else None
...
class TransactionSubmissionSchema(Schema):
    ...
    transaction_type = fields.Str(required=True, validate=validate.OneOf([e.value for e in TransactionType]))
...
class ContractDeploymentSchema(Schema):
    ...
    contract_type = fields.Str(required=True, validate=validate.OneOf([e.value for e in ContractType]))
