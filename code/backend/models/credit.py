...
class CreditScore(db.Model):
    ...
    status = db.Column(db.Enum(CreditScoreStatus), default=CreditScoreStatus.CALCULATING)
    ...
    calculation_method = db.Column(db.Text, nullable=True)  # JSON with calculation details
    
    # Validity
    calculated_at = db.Column(db.DateTime(timezone=True), default=datetime.now(timezone.utc))
    expires_at = db.Column(db.DateTime(timezone=True), nullable=True)
    valid_until = db.Column(db.DateTime(timezone=True), nullable=True)
    
    # Blockchain Integration
    blockchain_hash = db.Column(db.String(66), nullable=True, index=True)  # Transaction hash
    ...
    # Relationships
    factors = db.relationship('CreditFactor', backref='credit_score', cascade='all, delete-orphan')
    history = db.relationship('CreditHistory', backref='credit_score', cascade='all, delete-orphan')
...
class CreditFactor(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    credit_score_id = db.Column(db.String(36), db.ForeignKey('credit_scores.id'), nullable=False)
    ...
    contribution = db.Column(db.Float, nullable=True)  # Points contributed to total score
    
    # Factor Metadata
    data_source = db.Column(db.String(100), nullable=True)
    confidence_level = db.Column(db.Float, nullable=True)
    calculation_details = db.Column(db.Text, nullable=True)  # JSON with calculation details
...
class CreditHistory(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = db.Column(db.String(36), db.ForeignKey('users.id'), nullable=False, index=True)
    credit_score_id = db.Column(db.String(36), db.ForeignKey('credit_scores.id'), nullable=True)
    ...
    reported_date = db.Column(db.DateTime(timezone=True), default=datetime.now(timezone.utc))
...
class CreditFactorSchema(Schema):
    ...
    normalized_value = fields.Float(validate=validate.Range(min=0, max=1), dump_only=True)
    ...
    confidence_level = fields.Float(validate=validate.Range(min=0, max=1), dump_only=True)
    created_at = fields.DateTime(dump_only=True)
    updated_at = fields.DateTime(dump_only=True)
