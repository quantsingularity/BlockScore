...
class PerformanceMonitor:
    ...
    def get_metrics(self, name: str, start_time: datetime = None, 
                   end_time: datetime = None, tags: Dict[str, str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if tags:
                filtered_points = []
                for point in points:
                    if point.tags and all(point.tags.get(k) == v for k, v in tags.items()):
                        filtered_points.append(point)
                points = filtered_points
            ...
            for endpoint, times in self.response_times.items():
                if times:
                    ...
                    response_time_stats[endpoint] = {
                        ...
                        'p99': sorted_times[int(count * 0.99)] if count > 10 else sorted_times[-1]
                    }
    ...
    def add_alert_rule(self, name: str, metric_name: str, threshold: float, 
                      operator: str = 'gt', tags: Dict[str, str] = None,
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'error' not in system_metrics:
                if system_metrics['cpu']['percent'] > self.default_thresholds['cpu_percent']:
                    health_score -= 20
                    issues.append(f"High CPU usage: {system_metrics['cpu']['percent']:.1f}%")
                
                if system_metrics['memory']['percent'] > self.default_thresholds['memory_percent']:
                    health_score -= 20
                    issues.append(f"High memory usage: {system_metrics['memory']['percent']:.1f}%")
                
                if system_metrics['disk']['percent'] > self.default_thresholds['disk_percent']:
                    health_score -= 15
                    issues.append(f"High disk usage: {system_metrics['disk']['percent']:.1f}%")
            
            # Check application metrics
            if 'error' not in app_metrics:
                for endpoint, stats in app_metrics['error_rates'].items():
                    if stats['error_rate'] > self.default_thresholds['error_rate_percent']:
                        health_score -= 10
                        issues.append(f"High error rate for {endpoint}: {stats['error_rate']:.1f}%")
                
                for endpoint, stats in app_metrics['response_times'].items():
                    if stats['avg'] > self.default_thresholds['response_time_ms']:
                        health_score -= 10
                        issues.append(f"Slow response time for {endpoint}: {stats['avg']:.1f}ms")
    ...
    def _check_alerts(self, metric_name: str, value: float, tags: Dict[str, str] = None):
        ...
        for alert_name, rule in self.alert_rules.items():
            ...
            if triggered:
                ...
                self.logger.warning(f"Alert triggered: {alert_name} - {metric_name} {operator} {threshold} (actual: {value})")
            ...
            while self.system_metrics_enabled:
                try:
                    ...
                    if 'error' not in system_metrics:
                        ...
                        self.record_metric('system_cpu_percent', system_metrics['cpu']['percent'])
                        self.record_metric('system_memory_percent', system_metrics['memory']['percent'])
                        self.record_metric('system_disk_percent', system_metrics['disk']['percent'])
                        
                        # Record memory usage in bytes
                        self.record_metric('system_memory_used_bytes', system_metrics['memory']['used'])
                        self.record_metric('system_memory_available_bytes', system_metrics['memory']['available'])
                        
                        # Record disk usage in bytes
                        self.record_metric('system_disk_used_bytes', system_metrics['disk']['used'])
                        self.record_metric('system_disk_free_bytes', system_metrics['disk']['free'])
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                metric_name = f"{metric_prefix}{func.__name__}_duration_ms" if metric_prefix else f"{func.__name__}_duration_ms"
                
                monitor.record_metric(metric_name, execution_time, {'status': 'success'})
                monitor.record_metric(f"{metric_prefix}{func.__name__}_calls_total" if metric_prefix else f"{func.__name__}_calls_total", 1, {'status': 'success'})
                ...
                monitor.record_metric(f"{metric_prefix}{func.__name__}_calls_total" if metric_prefix else f"{func.__name__}_calls_total", 1, {'status': 'error'})
                monitor.record_metric(f"{metric_prefix}{func.__name__}_errors_total" if metric_prefix else f"{func.__name__}_errors_total", 1, {'error_type': type(e).__name__})
...
def monitor_database_query(monitor: PerformanceMonitor):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # Try to extract query information from function name or args
                query_type = 'unknown'
                table = 'unknown'
