...
class CacheManager:
    ...
    def __init__(self, redis_client: Optional[redis.Redis] = None, 
                 default_ttl: int = 3600, key_prefix: str = "blockscore"):
    ...
    def set_multi(self, data: Dict[str, Any], ttl: Optional[int] = None,
                  serializer: str = 'json') -> bool:
...
def cached(cache_manager: CacheManager, ttl: int = 3600, 
          key_prefix: str = "", serializer: str = 'json'):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            cache_key = f"{key_prefix}:{cache_manager._hash_key(key_data)}" if key_prefix else cache_manager._hash_key(key_data)
        ...
        wrapper.cache_clear = lambda: cache_manager.clear_pattern(f"{key_prefix}:*" if key_prefix else "*")
...
def cache_invalidate(cache_manager: CacheManager, patterns: List[str]):
        ...
        for user_id in user_ids:
            try:
                ...
                if not self.cache.exists(cache_key):
                    ...
                    user_data = {'id': user_id, 'cached_at': datetime.now(timezone.utc).isoformat()}
    ...
    def warm_credit_scores(self, user_ids: List[str]) -> Dict[str, Any]:
        ...
        for user_id in user_ids:
            try:
                ...
                if not self.cache.exists(cache_key):
                    # Fetch credit score data (placeholder)
                    score_data = {'user_id': user_id, 'score': 750, 'cached_at': datetime.now(timezone.utc).isoformat()}
                    
                    if self.cache.set(cache_key, score_data, ttl=1800):  # 30 minutes
                        results['success'] += 1
                    else:
                        results['failed'] += 1
                
            except Exception as e:
                self.logger.error(f"Credit score cache warming failed for user {user_id}: {e}")
                results['failed'] += 1
