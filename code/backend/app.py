...
from models.user import User, UserProfile, UserSession, UserRegistrationSchema, UserLoginSchema
from models.credit import CreditScore, CreditHistory, CreditFactor, CreditScoreCalculationRequest
from models.loan import LoanApplication, Loan, LoanPayment, LoanApplicationSchema
from models.audit import AuditLog, ComplianceRecord, AuditEventType, AuditSeverity
from models.blockchain import BlockchainTransaction, SmartContract, TransactionType, TransactionStatus
...
def create_app(config_name='default'):
    ...
    def before_request():
        ...
        # Log request
        app.logger.info(f"[{g.request_id}] {request.method} {request.url} - IP: {request.remote_addr}")
    ...
    def after_request(response):
        ...
        if hasattr(g, 'start_time'):
            ...
            # Log response
            app.logger.info(f"[{g.request_id}] Response: {response.status_code} - Time: {response_time:.2f}ms")
    ...
    def health_check():
        ...
        return jsonify({
            ...
            'services': {
                ...
                'redis': 'up' if redis_status else 'down' if redis_client else 'not_configured',
    ...
    def apply_for_loan():
        ...
        try:
            ...
            current_score = CreditScore.query.filter_by(user_id=user_id)\
            ...
            if current_score:
                ...
                application.approval_probability = min(100, (base_probability * 0.7 + amount_factor * 0.3) * 100)
