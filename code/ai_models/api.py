...
model_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'credit_scoring_model.pkl')
...
def preprocess_blockchain_data(credit_history):
    ...
    for record in credit_history:
        ...
        if record['repaid']:
            ...
            if record['repaymentTimestamp'] > 0:
                days_to_repay = (int(record['repaymentTimestamp']) - int(record['timestamp'])) / (60 * 60 * 24)
    ...
    active_debt = total_borrowed - (repaid_count / total_records * total_borrowed) if total_records > 0 else 0
    ...
    credit_utilization = min(avg_repayment_time / 90, 1.0)  # Normalize to 0-1, capped at 90 days
