...
from model_integration import transform_blockchain_data, predict_score, calculate_score_factors
...
def batch_predict():
    ...
    try:
        data = request.get_json()
        logger.info(f"Received batch prediction request with {len(data.get('batch', []))} items")
        
        if not data or 'batch' not in data:
            logger.warning("Invalid input data: missing batch field")
            return jsonify({
                'error': 'Invalid input data. Expected "batch" field.'
            }), 400
