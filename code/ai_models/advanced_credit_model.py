...
class AdvancedCreditScoringModel:
    ...
    def _generate_comprehensive_synthetic_data(
        ...
        # Credit history
        credit_history_length = np.random.exponential(
            8, n_samples
        )  # Years of credit history
        credit_history_length = np.clip(credit_history_length, 0, age - 18)

        # Payment behavior
        payment_history_score = np.random.beta(
            8, 2, n_samples
        )  # Skewed towards good payment history
        ...
        credit_utilization = np.random.beta(
            2, 5, n_samples
        )  # Skewed towards lower utilization
